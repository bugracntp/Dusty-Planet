...
function parseData(){
    return fetch(urls().weather).
    then(data=> {return data.json();}).then(data =>{
        for(var i = data.soles.length-10; i >= 0; i--)
            loaddata(data.soles[i])
    })
}	
...
function loaddata(solObject){
    ...
    let highFahrenheit = isNaN(solObject.max_temp)? "nodata" : Math.round(highCelsius * 9/5 + 32);
    let lowFahrenheit = isNaN(solObject.min_temp)? "nodata" : Math.round(lowCelsius * 9/5 + 32);
